...
Alembic plans operations over a tree of ops. You bring your own op types and your own rules;
Alembic provides the machinery that matches rules against the tree, applies them, and drives the
rewrite to a result. It attaches no meaning to your ops — they may model a query, a data
...
- **Rules.** Strongly-typed pattern matching over subtrees, including *converter rules* that lower a
  op from one convention to another.
...
Define your op types and a rule, then run the planner:
...
Early and evolving. The current release provides the op model, interned trait sets, conventions
