...
The shared environment for a single planning session. Every node built during that session carries a
reference to the same cluster, and the cluster carries a reference to the **planner** that will optimize
those nodes. It is the thread that ties a tree of nodes to the engine working on them.

Concretely a cluster holds:

- **The planner.** A node reaches its planner through its cluster (`node.Cluster.Planner`). This is how a
  node-side operation that needs the engine — for example, a physical node converting one of its inputs
  to a required trait set — gets hold of it without being passed the planner explicitly. Because of this,
  the invariant is strict: a node's cluster must point at the planner that actually optimizes it. (Tests
...
- **The default trait set.** The cluster exposes a baseline `TraitSet` — by default the planner's empty
  ...
  (`TraitSetOf`) to produce that baseline with specific traits applied. Nodes use this as the starting
...
metadata cache; Alembic's carries neither). It is just "the planner, as seen from inside a node," plus
