...
| Member | Verdict | Note |
|---|---|---|
| `onRegister` | **RESOLVED (ported)** | extracted onto `IOp.OnRegister` / `AbstractOp.OnRegister` as a faithful port of `AbstractRelNode.onRegister`: register each input via `planner.EnsureRegistered` (promoted to the `IOpPlanner` interface, as Calcite's `RelOptPlanner.ensureRegistered`; `HepPlanner.EnsureRegistered` returns the op unchanged, as Calcite's does), copy if any input changed, recompute the digest. The convention coercion that had been fused in is split out into the Alembic-original `VolcanoPlanner.CoerceInputConventions`, called right after `OnRegister` in `RegisterImpl` — Calcite distributes that lowering to each physical rule's `convert()` call; Alembic centralizes it, consistent with its rule-driven convention-conversion divergence (DIVERGENT-OK, below). |
